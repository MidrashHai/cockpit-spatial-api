...
function compute(lat, lon) {
  ...
  const mIdx   = seqLat[0];
  const sIdx   = seqLon[0];
  const mshIdx = (seqLat[seqLat.length-1] + seqLon[seqLon.length-1]) % 10;
  ...
  return {
    ...
    icl: {
      ...
      territoire:   seqLat.join(''),
      porte:        seqLon.join(''),
      lettres:      iclLat + '|' + iclLon,
      guematria:    { g_lat: gLat, g_lon: gLon, g_total: gTotal },
      graine: {
        idx_lat: graineLatIdx, idx_lon: graineLonIdx,
        lettre_lat: LM[graineLatIdx].let, lettre_lon: LM[graineLonIdx].let,
        heb: shemGraine.heb, lat: shemGraine.lat, cellule_m: 10,
      },
      mishkan_index: {
        calcul: `(${graineLatIdx}+${graineLonIdx}) mod 10 = ${mishkanIdx}`,
        index:  mishkanIdx, shem: mishkanShem.l, heb: mishkanShem.h,
        fr: mishkanShem.fr, sig: mishkanShem.s,
      },
      coordonnees: `${Math.abs(lat).toFixed(7)}° ${ns} · ${Math.abs(lon).toFixed(7)}° ${eo}`,
      source: 'GPS', regime: 'EXP',
    },
    ...
    ponderation: {
      val_lat: parseFloat(valLat.toFixed(4)), seq_lat: seqLat, lecture_lat: lectureLat,
      val_lon: parseFloat(valLon.toFixed(4)), seq_lon: seqLon, lecture_lon: lectureLon,
    },
